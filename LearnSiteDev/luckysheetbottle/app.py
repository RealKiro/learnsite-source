...
class WebSocketManager:
    ...
    def register(cls, ctx):
        with cls.lock:
            ...
            uid=f"user-{cls.index}-{ctx.address[0].split('.')[-1]}" 
            cls.pool[uid] = ctx
            return uid
...
def testHandler(ctx: pywss.Context):
    Qip="192.168.1.3"
    result=linksql(Qip)
    ctx.write("hello test "+result)


def lucky(ctx: pywss.Context):
    req = ctx.url_params
    print(req)
    if('name' in req):
        ctx.write(req['name'])
...
def main(defaultPort = 8080):
    ...
    app.get("/test", testHandler)  # 匿名函数也是不错的选择
    # 启动服务
